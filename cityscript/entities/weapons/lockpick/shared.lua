...
SWEP.Instructions = TEXT.LockPickInstructions
SWEP.Contact = ""
SWEP.Purpose = ""
...
function SWEP:PrimaryAttack()
	...
	if e ~= nil then
		if (trace.HitPos:Distance(self.Owner:GetShootPos()) <= 75 and not CAKE.IsDoor(e) and not e:IsVehicle()) then
			self.Weapon:SendWeaponAnim(ACT_VM_HITCENTER)
			self.Owner:FireBullets(bullet)
			self.Weapon:EmitSound("physics/flesh/flesh_impact_bullet" .. math.random(3, 5) .. ".wav")
		elseif (IsValid(e) and (trace.HitPos:Distance(self.Owner:GetShootPos()) <= 75 and (CAKE.IsDoor(e) or e:IsVehicle()))) then
			self.Weapon:SendWeaponAnim(ACT_VM_HITCENTER)
			self.Owner:FireBullets(bullet)
			self.Weapon:EmitSound("physics/flesh/flesh_impact_bullet" .. math.random(3, 5) .. ".wav")
