...
function daylight:buildLightTable()
	...
	for i = 1, DAY_LENGTH do
		...
		if i >= DAY_START and i < NOON then
		...
		elseif i >= NOON and i < DAY_END then
...
function daylight:think()
	-- not ready to think?
	if not self.ready or self.nextthink > CurTime() then return; end
	...
	if CAKE.ClockDay and (CAKE.ClockDay >= get_days_in_month(CAKE.ClockMonth)) then 
		CAKE.ClockMonth = CAKE.ClockMonth + 1
		CAKE.ClockDay = 1
	end

	if CAKE.ClockMonth and CAKE.ClockMonth > 12 then 
	...
	if self.light_environment and self.pattern ~= pattern then
		local light
		for _, light in pairs(self.light_environment) do
	...
	if self.env_sun and self.env_sun_angle ~= env_sun_angle then
		local sun
		for _, sun in pairs(self.env_sun) do
...
function daylight:checkNightLight(ent , key , val)
	...
	if not string.find(ent:GetClass(), "light") then return; end
...
function daylight:lightsOn()
	-- no lights? bail.
	if not self.nightlights then return; end
	...
	-- loop through lights and turn them on.
	local light
