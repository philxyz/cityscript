...
function ITEM:UseItem(ply)
	...
	for k, v in pairs(ents.FindInSphere(self:GetPos(), 10)) do
		...
		elseif IsValid(v) and v:GetClass() == "prop_ragdoll" and v.isdeathdoll and v.ply:GetNWInt("deathmode") == 1 and v.ply ~= ply then
	...
	if #possible > 1 then
		CAKE.Response(ply, "*beep* Multiple targets found")
		return
	elseif #possible == 0 then
		CAKE.Response(ply, "*beep* No target found")
		return
	end
